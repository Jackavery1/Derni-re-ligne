...
**Version : 2.5.34** — détails dans [CHANGELOG.md](CHANGELOG.md).
...
**Campagne complète (audit D9, ~60 min)** : `npm run test:e2e:d9` — sous-tests D9b dans `npm run test:e2e:audit` ; parcours intégral en CI nightly (`e2e-d9-nightly.yml`, voir [CONTRIBUTING.md](CONTRIBUTING.md)).

**Dev :** préférer `127.0.0.1` à `localhost` (HSTS). Après MAJ du SW : Ctrl+Shift+R. Debug : `?debug=1`. PWA locale : `?pwa=1` (SW actif en dev ; sinon désactivé sur localhost — voir `js/io/sw-dev.js`).
