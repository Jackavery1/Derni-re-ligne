...
test(
    'journal mobile ultra-etroit 319px — contenu scrollable (audit D8)',
    { tag: '@viewport-mobile-etroit' },
    async ({ page }) => {
...
test(
    'carte histoire 319px — overlay objectifs pre-partie lisible (audit D8)',
    { tag: '@viewport-mobile-etroit' },
    async ({ page }) => {
        await page.setViewportSize({ width: 319, height: 568 });
        ...
        await expect(page.locator('#overlay-objectifs-pre')).toHaveClass(
...
test(
    'recap post-monde paysage mobile — panneau scrollable',
        ...
        const metriques = await page.evaluate(() => {
            ...
            return {
                ...
                panneauDansEcran: Boolean(
                    panneauRect &&
                    panneauRect.bottom <= window.innerHeight + 2 &&
                    panneauRect.top >= -2
                ),
...
test(
    'boss HUD 480px — portrait visible sans debordement (audit D8)',
...
test(
    'recap post-monde portrait 319px — panneau scrollable (audit D8)',
    { tag: '@viewport-mobile-etroit' },
    async ({ page }) => {
        test.setTimeout(60000);
        await page.setViewportSize({ width: 319, height: 568 });
...
test(
    'iphone — recap post-victoire respecte encoche simulee (audit D8/D11)',
    { tag: '@viewport-mobile-portrait' },
    async ({ browser }) => {
        test.setTimeout(60000);
...
test(
    'journal mobile paysage — contenu scrollable (audit D8)',
    { tag: '@viewport-mobile-landscape' },
    async ({ page }) => {
        test.setTimeout(90000);
        await page.setViewportSize({ width: 844, height: 390 });
...
test(
    'carte histoire paysage mobile — header et retour accessibles',
    { tag: '@viewport-mobile-landscape' },
    async ({ page }) => {
        await page.setViewportSize({ width: 844, height: 390 });
        await ouvrirCarteHistoire(page);
