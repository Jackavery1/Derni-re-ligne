...
export function dessinerFondBiome() {
    ...
    if (store.modeHistoireActif && BIOMES[obtenirBiomeActif()]?.mecaniqueSpeciale === 'eclipse') {
        _dessinerLigneEclipse();
    }
    if (store.modeHistoireActif && BIOMES[obtenirBiomeActif()]?.mecaniqueSpeciale === 'trame') {
        _dessinerFondTrame();
        _dessinerFlickerTrame();
    }
}

function _dessinerLigneEclipse() {
    const ctx = obtenirCtx();
...
function _dessinerFlickerTrame() {
    const ctx = obtenirCtx();
    const canvas = obtenirCanvasPlateau();
    const t = performance.now() / 1000;
